...
def create_app(
    ...
    call_dispatcher = dispatcher or (
        DryRunDispatcher() if config.dry_run else LiveKitDispatcher(config)
    )
    ...
    async def create_call(request: CallRequest) -> CallResponse:
        ...
        if latest:
            ...
            if now - latest_at < timedelta(days=config.recipient_cooldown_days):
        ...
        database.create_call(
            {
                ...
                "consent_basis": request.consent_basis.value,
        ...
        return CallResponse(
            ...
            created_at=created_at,
            policy_summary=(
                f"{phone_decision.reason}; {time_decision.reason}; "
                f"consent={request.consent_basis.value}"
            ),
        )
    ...
    def add_opt_out(request: SuppressionRequest) -> None:
        ...
        database.add_suppression(
            phone_hash(request.phone_number, config.phone_hash_pepper),
